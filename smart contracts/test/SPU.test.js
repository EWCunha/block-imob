...
describe ("SPU Hackathon tests", () => {
    ...
    it("BLOCKCIMOB", async() => {
        await blockimob.connect(accounts[0]).mint(accounts[0].address,"uri test","Vila Velha",0001);

        assert(NFTimob.balanceOf(accounts[0]),1);

        console.log("Mintado posição: 1")
        console.log("URI: ", await blockimob.tokenURI(1))
        console.log("URI from query {Vila Velha,0001}: ", await blockimob.uriFromQuery("Vila Velha",0001))
